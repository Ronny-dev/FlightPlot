...
public class FlightPlot {
    ...
    private static String appName = "FlightPlot魔改版";
    private static String version = "0.3.2_Alpha";
    ...
    private final static String lonFields = "vehicle_global_position_0.lon";
    private final static String latFields = "vehicle_global_position_0.lat";

    private XYSeries latSeries;
    private XYSeries lonSeries;
        ...
        openMapButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (latSeries != null && lonSeries != null) {
                    if (mapView == null) {
                        mapView = new MapView(latSeries, lonSeries);
                    }
                    mapView.show();
                }
            }
        ...
        chartPanel.addChartMouseListener(new ChartMouseListener() {
            ...
            public void chartMouseClicked(ChartMouseEvent chartMouseEvent) {
                if (chartMouseEvent.getEntity() instanceof XYItemEntity) {
                    ...
                    if (entity.getDataset() instanceof XYSeriesCollection) {
                        XYSeriesCollection seriesCollection = (XYSeriesCollection) entity.getDataset();
                        if (seriesCollection.getSeriesCount() > 0) {
                            float v = seriesCollection.getX(0, item).floatValue();
                            System.out.println(v);
                            if (mapView != null) {
                                mapView.setLocationTag(v);
                            }
                        }
                    }
                }
            ...
            for (int i = 0; i < activeProcessors.size(); i++) {
                ...
                for (PlotItem item : processor.getSeriesList()) {
                    if (item instanceof Series) {
                        ...
                        if (jseries.getKey().toString().contains(latFields)) {
                            latSeries = jseries;
                        }
                        if (jseries.getKey().toString().contains(lonFields)) {
                            lonSeries = jseries;
                        }
